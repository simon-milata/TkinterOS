...
def test_start_with_special_character():
    for char in punctuation:
        file_name = char + "a"
        assert FileManager().validate_file_name_on_creation(file_name=file_name, files=[]) == False
...
def test_file_already_exists():
    ...
    assert FileManager().validate_file_name_on_creation(file_name=file_name, files=files) == False
...
def test_name_starts_with_number():
    file_name = "4A.txt"
    assert FileManager().validate_file_name_on_creation(file_name=file_name, files=[]) == True
